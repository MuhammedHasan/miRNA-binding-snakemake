...
gr_partial = gr[~exact_match]
gr_partial.df.to_csv(snakemake.output['partial'])

gr = gr[exact_match]


df_families = pd.read_csv(snakemake.input['families'], sep='\t')
cols = df_families.columns
...
df_families['miR family'] = df_families['gene_id']

df_families \
    .rename(columns={'index': 'MiRBase Accession'})[cols] \
    .to_csv(snakemake.output['families'], sep='\t', index=False)
